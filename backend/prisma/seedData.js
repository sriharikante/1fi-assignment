...
export default function ProductPage() {
  ...
  /*
   * Calculate EMI plans for the currently selected variant.
   *
   * EMI plans are stored against the product's base price.
   * When the user selects another storage variant, the price changes,
   * so the monthly payment is scaled according to the new variant price.
   */
  const calculatedEmiPlans = useMemo(() => {
    ...
    if (!basePrice || !variantPrice) {
      return product.emiPlans;
    }

    const priceRatio = variantPrice / basePrice;

    return product.emiPlans.map((plan) => ({
      ...plan,
      monthlyPayment: Math.round(
        Number(plan.monthlyPayment) * priceRatio
      ),
    }));
  ...
  function handleColorChange(color) {
    ...
    // Reset selected EMI because the variant may have changed.
    setSelectedPlanId(null);
  }
  ...
  function handleStorageChange(storage) {
    ...
    // Reset selected EMI because the variant price changed.
    setSelectedPlanId(null);
  }
        ...
        <div>
          ...
          {selectedPlan && (
            <div className="mt-4 rounded-2xl bg-cashback-soft px-4 py-3 text-sm text-cashback">
              ...
              {formatINR(selectedPlan.cashback)} cashback on completion.
            </div>
          )}
