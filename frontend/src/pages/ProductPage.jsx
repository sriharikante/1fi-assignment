...
export default function ProductPage() {
  ...
  const selectedPlan = useMemo(
    () => product?.emiPlans.find((p) => p.id === selectedPlanId) || null,
    [product, selectedPlanId]
  );

  function handleColorChange(color) {
    setSelectedColor(color);
    // Keep the same storage if that combination exists, otherwise fall back
    // to the first variant available in the newly selected color.
    ...
    return (
      <main className="mx-auto max-w-md px-5 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-ink-soft">
          We couldn't find a product at "{slug}". It may have been removed or the link is
          incorrect.
        </p>
        <Link
          to="/"
  ...
  const discount = calcDiscountPercent(product.mrp, activeVariant.price);

  return (
    <main className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-12">
      <Link to="/" className="mb-7 inline-flex items-center gap-2 text-sm font-semibold text-ink-soft transition hover:text-brand">← Back to phones</Link>
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-12">
        ...
        <div>
          <div className="relative aspect-square overflow-hidden rounded-[30px] border border-line bg-gradient-to-br from-[#f7f7f5] to-[#eceff4] shadow-sm">
            ...
            />
            <div className="absolute inset-0 hidden items-center justify-center text-sm font-semibold text-ink-faint">Image unavailable</div>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-4 max-w-xl text-sm leading-6 text-ink-soft sm:text-base">{product.description}</p>
          ...
          <div className="mt-7 rounded-3xl border border-line bg-surface p-5 shadow-sm space-y-5">
            <ColorSelector
              colors={colors}
          ...
          <div className="mt-5 rounded-3xl border border-line bg-surface p-5 shadow-sm">
            <div className="mb-4 flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-brand">Flexible financing</p><p className="mt-1 text-sm font-bold text-ink">Choose your EMI plan</p></div><span className="text-xs text-ink-faint">{product.emiPlans.length} plans</span></div>
          ...
          {selectedPlan && <div className="mt-4 rounded-2xl bg-cashback-soft px-4 py-3 text-sm text-cashback"><span className="font-bold">You save with this plan.</span> {formatINR(selectedPlan.cashback)} cashback on completion.</div>}
          ...
          >
            {selectedPlan ? "Proceed" : "Select an EMI plan to proceed"}
          </button>
        </div>
      </div>
